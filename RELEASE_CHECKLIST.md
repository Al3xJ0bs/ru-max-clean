...
5. Проверить, что архив содержит только программу, тесты и документацию; корпусный
   сканер и его отчёты остаются внутренними инструментами.
6. Посчитать SHA-256 и сохранить его в тексте GitHub Release.
7. Создать tag `vX.Y.Z` и опубликовать только builder ZIP как Release asset.
...
Готовые `.dict/.idx/.ifo`, production-архивы, большие кэши, reader-пакеты и
корпусные инструменты анализа книг в GitHub Releases не публикуются. GitHub автоматически добавляет архивы исходного
